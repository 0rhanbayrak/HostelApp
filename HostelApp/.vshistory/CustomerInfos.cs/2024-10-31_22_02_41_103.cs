...
namespace HostelApp
{
    public partial class CustomerInfos : Form
    {
        ...
        {
            link.Open();
            SqlCommand cmd = new SqlCommand("SELECT * FROM CUSTOMERINFO WHERE Name LIKE @pWantedId", link);
            cmd.Parameters.AddWithValue("@pWantedId", "%" + textBoxSearch.Text + "%");

            SqlDataReader reader = cmd.ExecuteReader();

            // DataGridView ile sonuçları göstermek için tablo oluşturup doldurabilirsiniz
