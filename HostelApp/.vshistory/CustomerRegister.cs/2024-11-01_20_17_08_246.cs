...
namespace HostelApp
{
    public partial class CustomerRegister : Form
    {
        ...
        private void LoadButtonStates()
        {
            ...
            {
                connection.Open();
                string query = "SELECT ButtonID, IsOccupied FROM ButtonStates";
                SqlCommand command = new SqlCommand(query, connection);
                SqlDataReader reader = command.ExecuteReader();

                while (reader.Read())
                {
                    string buttonID = reader["ButtonID"].ToString();
                    bool isOccupied = (bool)reader["IsOccupied"];

                    // Düğme bulunursa rengini ayarla
                    Button button = this.Controls.Find(buttonID, true).FirstOrDefault() as Button;
                    if (button != null)
                    {
                        button.BackColor = isOccupied ? Color.Red : Color.LawnGreen;
                    }
                }
        ...
        // Düğmenin durumunu değiştirme ve veritabanına kaydetme
        private void ToggleButtonState(Button button, string buttonID)
        {
            if (button.BackColor == Color.LawnGreen)
            {
                button.BackColor = Color.Red;
                SaveButtonState(buttonID, true);  // Durum 'dolu' olarak kaydedilir
        ...
        private void SaveButtonState(string buttonID, bool isOccupied)
        {
            using (SqlConnection connection = new SqlConnection(connectionString))
            {
                connection.Open();
                string query = "UPDATE ButtonStates SET IsOccupied = @IsOccupied WHERE ButtonID = @ButtonID";
                SqlCommand command = new SqlCommand(query, connection);
