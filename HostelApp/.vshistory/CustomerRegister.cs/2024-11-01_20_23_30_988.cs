...
namespace HostelApp
{
    public partial class CustomerRegister : Form
    {
        ...
        private void LoadButtonStates()
        {
                ...
                link.Open();
                string query = "SELECT ButtonName, IsOccupied FROM BUTTONINFO";
                SqlCommand command = new SqlCommand(query, link);
                SqlDataReader reader = command.ExecuteReader();

                while (reader.Read())
                {
                    string buttonID = reader["ButtonName"].ToString();
        ...
        private void ToggleButtonState(System.Windows.Forms.Button button, string buttonID)
        {
        ...
        private void SaveButtonState(string ButtonName, bool isOccupied)
        {
                ...
                link.Open();
                string query = "UPDATE BUTTONINFO SET IsOccupied = @IsOccupied WHERE ButtonName = @ButtonID";
                SqlCommand command = new SqlCommand(query, link);
                command.Parameters.AddWithValue("@IsOccupied", isOccupied ? 1 : 0);
                command.Parameters.AddWithValue("@ButtonID", ButtonName);
